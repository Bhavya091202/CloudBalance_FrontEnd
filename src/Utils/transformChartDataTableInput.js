export const transformDbDataToTableChart = (dbData, groupByField) => {
  if (!dbData || dbData.length === 0 || !groupByField)
    return { columns: [], rows: [] };
  ...
  dbData.forEach((item) => {
    ...
    monthMap[month][groupValue] =
      (monthMap[month][groupValue] || 0) + (item.TOTAL_USAGE_COST || 0);
  });

  const allGroups = Array.from(
    new Set(dbData.map((item) => item[groupByField] || "Unknown"))
  );
  ...
  const columns = [
    {
      key: groupByField,
      label:
        groupByField.charAt(0).toUpperCase() +
        groupByField.slice(1).toLowerCase(),
    },
    ...categories.map((cat) => ({
      key: cat.label,
      label: cat.label,
